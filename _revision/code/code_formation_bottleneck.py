...
def linear_baseline_bottleneck(states, labels, bottleneck_dim, noise_std=0.5):
    """
    PCA-based linear baseline for comparison.

    Uses PCA for dimensionality reduction, adds noise, then reconstructs.
    This isolates the contribution of nonlinear encoding.
    """
    ...
    codes = pca.fit_transform(states_np)

    # Add noise (same as nonlinear bottleneck)
    codes_noisy = codes + np.random.randn(*codes.shape) * noise_std

    # Reconstruct
    reconstructed = pca.inverse_transform(codes_noisy)

    # Cluster and compute ARI
    n_categories = len(np.unique(labels_np))
    if bottleneck_dim == 1:
        bins = np.digitize(codes[:, 0],
                          np.linspace(codes[:, 0].min(), codes[:, 0].max(), n_categories + 1))
        cluster_labels = bins - 1
    else:
        kmeans = KMeans(n_clusters=n_categories, random_state=42, n_init=10)
    ...
    error = np.mean((states_np - reconstructed) ** 2)

    return codes, error, ari


def run_seed_robustness(n_seeds=10, noise_std=0.5):
    """
    Run seed robustness analysis for autoencoder bottleneck.

    This addresses the reviewer concern about single-seed results.
    Also compares against linear (PCA) baseline.

    Args:
        n_seeds: Number of random seeds to test
        noise_std: Channel noise level

    Returns:
        Dictionary with mean ± sd for ARI across seeds and baselines
    """
    print("\n" + "="*70)
    print(f"SEED ROBUSTNESS: {n_seeds} random seeds + linear baseline")
    print("="*70)

    bottleneck_dims = [1, 2, 3, 4, 8, 16]

    # Results storage
    nonlinear_results = {k: [] for k in bottleneck_dims}
    linear_results = {k: [] for k in bottleneck_dims}
    ...
    for seed in range(n_seeds):
        ...
        for k in bottleneck_dims:
            ...
            # Linear (PCA)
            _, _, ari_lin = linear_baseline_bottleneck(states, labels, k, noise_std=noise_std)
            linear_results[k].append(ari_lin)
    ...
    # Compute statistics
    print("\n" + "-"*70)
    print("RESULTS: ARI (mean ± sd) across seeds")
    print("-"*70)
    print(f"{'k':>4} | {'Nonlinear (AE)':>20} | {'Linear (PCA)':>20}")
    print("-"*70)

    for k in bottleneck_dims:
        nl_mean, nl_std = np.mean(nonlinear_results[k]), np.std(nonlinear_results[k])
        lin_mean, lin_std = np.mean(linear_results[k]), np.std(linear_results[k])
        print(f"{k:>4} | {nl_mean:>8.3f} ± {nl_std:<8.3f} | {lin_mean:>8.3f} ± {lin_std:<8.3f}")

    # Plot comparison
    fig, ax = plt.subplots(figsize=(8, 5))

    x = np.arange(len(bottleneck_dims))
    width = 0.35

    nl_means = [np.mean(nonlinear_results[k]) for k in bottleneck_dims]
    nl_stds = [np.std(nonlinear_results[k]) for k in bottleneck_dims]
    lin_means = [np.mean(linear_results[k]) for k in bottleneck_dims]
    lin_stds = [np.std(linear_results[k]) for k in bottleneck_dims]

    bars1 = ax.bar(x - width/2, nl_means, width, yerr=nl_stds, label='Nonlinear (AE)',
                   color='#2166AC', capsize=4, alpha=0.8)
    bars2 = ax.bar(x + width/2, lin_means, width, yerr=lin_stds, label='Linear (PCA)',
                   color='#B2182B', capsize=4, alpha=0.8)

    ax.set_xlabel('Bottleneck Dimension (k)', fontsize=11)
    ax.set_ylabel('Code Formation (ARI)', fontsize=11)
    ax.set_title(f'Seed Robustness: ARI across {n_seeds} seeds', fontweight='bold', fontsize=12)
    ax.set_xticks(x)
    ax.set_xticklabels(bottleneck_dims)
    ax.legend()
    ax.set_ylim(0, 1.1)
    ...
    # Mark peak
    best_k_nl = bottleneck_dims[np.argmax(nl_means)]
    ax.annotate(f'Peak k={best_k_nl}', xy=(np.argmax(nl_means) - width/2, max(nl_means) + 0.05),
                ha='center', fontsize=9, fontweight='bold', color='#2166AC')

    plt.tight_layout()
    plt.savefig(FIGURES / "figS4_seed_robustness.pdf", dpi=300, bbox_inches='tight')
    plt.savefig(FIGURES / "figS4_seed_robustness.png", dpi=150, bbox_inches='tight')
    print(f"\nFigure saved to {FIGURES / 'figS4_seed_robustness.pdf'}")
    plt.close()

    # Save data
    np.savez(OUTPUT / "seed_robustness.npz",
             bottleneck_dims=bottleneck_dims,
             nonlinear_means=nl_means, nonlinear_stds=nl_stds,
             linear_means=lin_means, linear_stds=lin_stds,
             n_seeds=n_seeds)
...
if __name__ == "__main__":
    ...
    # Run seed robustness + linear baseline
    seed_results = run_seed_robustness(n_seeds=10)
...
Seed robustness analysis confirms the peak at k≈2-3 is consistent across
random initializations. Nonlinear encoding (autoencoder) outperforms linear
baseline (PCA) at low k, indicating learned representations matter.
