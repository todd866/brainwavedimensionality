...
def run_ensemble_robustness(n_draws=50, N=2500, n_modules=25, p_within=0.3, p_between=0.01):
    """
    Run ensemble robustness analysis over many random network draws.

    This addresses the reviewer concern about single-realization results.

    Args:
        n_draws: Number of random network realizations
        N: Total nodes
    ...
    Returns:
        Dictionary with mean ± sd for key metrics
    """
    print("\n" + "="*70)
    print(f"ENSEMBLE ROBUSTNESS: {n_draws} random modular network draws")
    print("="*70)
    ...
    for seed in range(n_draws):
        ...
        # Create network with this seed
        G = nx.stochastic_block_model(sizes, probs, seed=seed)
        L = nx.laplacian_matrix(G).astype(float)

        # Compute eigenmodes
        eigenvalues, eigenvectors = eigsh(L, k=100, which='SM')
        idx = np.argsort(eigenvalues)
        eigenvalues = eigenvalues[idx][1:]  # Skip constant mode
        eigenvectors = eigenvectors[:, idx][:, 1:]

        # Compute PR for each mode
        ...
        # Compute metrics
        r = np.corrcoef(freq_norm, participation)[0, 1]
        slow_pr_mean = participation[:15].mean()
        fast_pr_mean = participation[-15:].mean()
        pr_ratio = slow_pr_mean / fast_pr_mean

        correlations.append(r)
        pr_ratios.append(pr_ratio)
        slow_prs.append(slow_pr_mean)
        fast_prs.append(fast_pr_mean)

        if (seed + 1) % 10 == 0:
            print(f"  Completed {seed + 1}/{n_draws} draws...")

    correlations = np.array(correlations)
    pr_ratios = np.array(pr_ratios)
    slow_prs = np.array(slow_prs)
    fast_prs = np.array(fast_prs)

    results = {
        'r_mean': correlations.mean(),
        'r_std': correlations.std(),
        'pr_ratio_mean': pr_ratios.mean(),
        'pr_ratio_std': pr_ratios.std(),
        'slow_pr_mean': slow_prs.mean(),
        'slow_pr_std': slow_prs.std(),
        'fast_pr_mean': fast_prs.mean(),
        'fast_pr_std': fast_prs.std(),
    ...
    print("-"*70)
    print(f"  Correlation (eigenvalue, PR):  r = {results['r_mean']:.3f} ± {results['r_std']:.3f}")
    print(f"  PR ratio (slow/fast):          {results['pr_ratio_mean']:.2f} ± {results['pr_ratio_std']:.2f}")
    print(f"  Slow modes mean PR:            {results['slow_pr_mean']:.1f} ± {results['slow_pr_std']:.1f}")
    print(f"  Fast modes mean PR:            {results['fast_pr_mean']:.1f} ± {results['fast_pr_std']:.1f}")

    # Plot ensemble distribution
    fig, axes = plt.subplots(1, 2, figsize=(10, 4))

    ax1 = axes[0]
    ax1.hist(correlations, bins=15, alpha=0.7, color='#2E7D32', edgecolor='white')
    ax1.axvline(results['r_mean'], color='red', linestyle='--', linewidth=2,
                label=f'mean = {results["r_mean"]:.3f}')
    ax1.set_xlabel('Correlation (eigenvalue, PR)')
    ax1.set_ylabel('Count')
    ax1.set_title(f'A. Correlation Distribution (n={n_draws})', fontweight='bold')
    ax1.legend()

    ax2 = axes[1]
    ax2.hist(pr_ratios, bins=15, alpha=0.7, color='#1565C0', edgecolor='white')
    ax2.axvline(results['pr_ratio_mean'], color='red', linestyle='--', linewidth=2,
                label=f'mean = {results["pr_ratio_mean"]:.2f}')
    ax2.set_xlabel('PR Ratio (slow 15 / fast 15)')
    ax2.set_ylabel('Count')
    ax2.set_title(f'B. Slow/Fast PR Ratio (n={n_draws})', fontweight='bold')
    ax2.legend()

    plt.tight_layout()
    plt.savefig(FIGURES / "figS3_ensemble_robustness.pdf", dpi=300, bbox_inches='tight')
    plt.savefig(FIGURES / "figS3_ensemble_robustness.png", dpi=150, bbox_inches='tight')
    print(f"\nEnsemble figure saved to {FIGURES / 'figS3_ensemble_robustness.pdf'}")
    plt.close()

    # Save data
    np.savez(OUTPUT / "ensemble_robustness.npz", **results)
...
if __name__ == "__main__":
    ...
    # Run ensemble robustness
    ensemble_results = run_ensemble_robustness(n_draws=50)
