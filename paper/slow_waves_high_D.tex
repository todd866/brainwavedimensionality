...
Beta & $k \approx 3$ & Floppy manifold & Manipulation, meta-cognition \\
...
\subsection{Maturity as Dimensional Capacity}

We further speculate that the capacity to sustain $k \geq 3$ dynamics---to hold contradictions, ambiguity, and nuance without forcing premature resolution---constitutes a geometric signature of cognitive and emotional maturity. The immature or stressed mind collapses to $k=2$: black/white, good/bad, us/them. The mature mind can inhabit the ``floppy'' space where paradoxes coexist.

Long-wavelength (slow) oscillations provide the \emph{temporal stability} required for collision-free representations. When slow-wave power is reduced (stress, sleep deprivation, developmental immaturity), the system loses the substrate needed to maintain $k \geq 3$ dynamics and defaults to rigid categorical processing.

\section{Methods}

\subsection{Laplacian Eigenmodes and Participation Ratio}

Graph Laplacian eigenmodes provide a natural basis for analysing spatially extended oscillatory patterns \citep{atasoy2016connectome}. Lower eigenvalues correspond to smoother, longer-wavelength modes; higher eigenvalues correspond to more localised, shorter-wavelength modes.
...
PR $\approx N$ when all oscillators contribute equally; PR $\approx 1$ when activity is confined to a single oscillator. We computed the 150 smallest eigenpairs using ARPACK.
...
Figure~\ref{fig:bottleneck} shows that code formation peaks at the critical bottleneck width $k=2$ (ARI = 0.88). At $k=1$, information is lost (ARI = 0.64). At $k \geq 3$, discretisation pressure diminishes and codes become more continuous (ARI = 0.85--0.87).

The critical $k=2$ reflects a topological constraint: the input categories are distinguished by phase relationships (cyclic, $S^1$ topology). Embedding a circle requires exactly 2 dimensions; 1D is insufficient, 3D is redundant.

\begin{figure}[ht]
    \centering
    \includegraphics[width=\textwidth]{../figures/fig2_code_formation_bottleneck.pdf}
    \caption{\textbf{Discrete codes emerge at critical capacity.} (A) Code formation (ARI) peaks at $k=2$, reflecting the $S^1$ topology of the phase-defined categories. While $k=3$ is topologically redundant for static embedding, Figure~\ref{fig:paradox} shows it is dynamically essential for non-intersecting trajectories. (B) Bottleneck codes at $k=2$ form six distinct clusters. (C) Schematic of compression mechanism.}
    \label{fig:bottleneck}
\end{figure}

\subsection{Self-Reference Requires $k \geq 3$}

Figure~\ref{fig:paradox} demonstrates collision dynamics. The Liar's Paradox trajectory produces 1511 self-intersections when confined to 2D (the system cannot distinguish ``going TRUE$\rightarrow$FALSE'' from ``returning FALSE$\rightarrow$TRUE''). In 3D, the trajectory forms a helix with zero self-intersections---the system can represent the \emph{process} of the paradox without confusion.

\begin{figure}[ht]
    \centering
    \includegraphics[width=\textwidth]{../figures/fig4_paradox_topology.pdf}
    \caption{\textbf{Self-referential logic requires $k \geq 3$.} (A) In 2D, the paradox trajectory collides with itself every cycle. (B) In 3D, the trajectory spirals upward (helix), avoiding collision. Time/meta-level provides the escape dimension.}
    \label{fig:paradox}
\end{figure}
...
\begin{itemize}
    ...
    \item \textbf{Beta oscillations ($k \approx 3$)}: Intermediate compression. Beta rhythms are associated with maintenance of the current cognitive state \citep{engel2010beta}. The manifold is constrained but remains ``floppy''---continuous enough for analog manipulation, mental rotation, holding ambiguity.
    \item \textbf{Gamma oscillations ($k \approx 2$)}: Tight compression. Forces categorical commitment---symbols, decisions, assertions. Optimal for transmission but sacrifices nuance.
\end{itemize}
...
\subsection{Maturity and Long-Wavelength Stability}

We speculate that the capacity to sustain $k \geq 3$ dynamics constitutes a geometric signature of cognitive and emotional maturity:
