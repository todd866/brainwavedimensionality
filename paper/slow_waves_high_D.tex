...
Notably, this motif---electric fields interacting with 2D surfaces---recurs across biology: cell membranes, bioelectric gradients in morphogenesis \citep{levin2021bioelectric}, and cortical sheets all share this geometric configuration. We do not claim universality, but the pattern suggests that 2D surfaces interfacing with volumetric fields may be a broadly useful architecture for biological information processing.

\subsection{Hierarchical Compression: Beta vs.\ Gamma}

The present model treats the bottleneck as a single stage, but the frequency hierarchy (delta $\rightarrow$ theta $\rightarrow$ beta $\rightarrow$ gamma) may implement a \emph{cascade} of progressively tighter bottlenecks. We speculate:

\begin{itemize}
    \item \textbf{Beta ($k \approx 3$):} A ``loose'' bottleneck that compresses the substrate enough to stabilise representations but preserves the continuous topology required for analog manipulation---holding and rotating a mental object without collapsing it into a fixed symbol.
    \item \textbf{Gamma ($k \approx 2$):} A ``tight'' bottleneck that forces discrete symbol formation for categorical decisions and error-corrected transmission.
\end{itemize}

This distinction has dynamical consequences. In a $k=2$ space, self-referential structures (e.g., logical paradoxes) produce trajectory collisions---the system cannot stably represent ``This statement is false'' because TRUE and FALSE occupy the same loop. In a $k=3$ space, the trajectory can lift into a helix, representing the \emph{process} of the paradox without collision. Beta may thus support meta-cognition (understanding paradoxes) while gamma supports definite assertion (resolving them---or failing to). The brain may dynamically adjust bottleneck width via frequency shifts to toggle between manipulation and commitment.

\subsection{Limitations and Measurement Requirements}

The modular network model captures the key feature of cortical organisation (dense local, sparse global connectivity) but does not model excitatory--inhibitory circuitry, conduction delays, or synaptic nonlinearities that shape real slow--gamma interactions; our goal here is to isolate the geometric and information-theoretic aspects of the problem. The bottleneck model is abstract and does not specify neural mechanisms for compression.

A preliminary test using scalp EEG (64-channel recordings, PhysioNet motor imagery dataset) did not show higher participation ratios for slow versus fast bands---in fact, the trend was reversed. This negative result is informative rather than disconfirming: scalp EEG measures volume-conducted potentials, not source-level activity. To detect the fine spatial structure of a slow wave, electrode spacing must be finer than the spatial wavelength of the oscillation. A delta wave coordinating activity across centimetres of cortex appears as a smooth, undifferentiated voltage gradient at the scalp; its high-dimensional \emph{source} geometry is invisible to coarse sensor arrays. Testing the participation ratio prediction requires dense intracortical recordings (e.g., Utah arrays, high-density ECoG) where electrode spacing resolves the spatial structure of the oscillation. This measurement constraint should guide future empirical tests.

\section{Conclusion}

Slow oscillations are geometrically high-dimensional: they coordinate many oscillators into coherent patterns that define a rich, continuous state space. When this high-dimensional substrate is communicated through bandwidth-limited, noisy channels, discrete codes emerge at critical capacity---the ``gamma bursts'' that carry working memory content.

This framework suggests a division of labour rather than a hierarchy:
\begin{itemize}
    \item \textbf{The Substrate (Analog):} The slow-wave state maintains a ``floppy,'' high-dimensional manifold---the continuous context necessary for motor fluidity, spatial navigation, and nuanced prediction. Not all computation benefits from discretisation.
    \item \textbf{The Code (Digital):} The gamma bottleneck enforces ``rigid'' discretisation when required---for categorical decisions, symbolic logic, and error-corrected transmission across noisy channels.
\end{itemize}

Intelligence may emerge from the controlled collapse of the former into the latter. The canvas is slow and deep; the brushstrokes are fast and superficial.
