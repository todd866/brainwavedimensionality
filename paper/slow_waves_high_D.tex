...
A striking feature of our bottleneck analysis is that optimal code formation occurs at $k=2$---a two-dimensional channel. This is precisely the dimensionality of the cortical sheet itself, which is fundamentally a 2D surface crumpled to fit within the 3D cranial volume. The coincidence is suggestive: perhaps the cortical surface area evolved to provide exactly the channel capacity needed for discrete code formation from high-dimensional volumetric dynamics. If so, surface area rather than volume would be the fundamental constraint on representational capacity---consistent with the observation that cortical folding (which increases surface area without increasing volume) correlates with cognitive complexity across species.
